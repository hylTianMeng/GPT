...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.c_attn = nn.Linear(config.n_embd, 3 * config.n_embd, bias=config.bias) # token 到每个 token 的 qkv
        self.c_proj = nn.Linear(config.n_embd, config.n_embd, bias=config.bias) # 拼接后的 z 到 y
